...
struct PostService {
    ...
    static func likePost(post: Post, completion: @escaping(FirestoreComplition)) {
        guard let uid = Auth.auth().currentUser?.uid else { return }
    ...
    static func unlikePost(post: Post, completion: @escaping(FirestoreComplition)) {
        guard let uid = Auth.auth().currentUser?.uid else { return }
    ...
    static func fetchFeedPosts(completion: @escaping([Post]) -> Void) {
        ...
        COLLECTION_USERS.document(uid).collection("user-feed").getDocuments { snapshot, error in
            snapshot?.documents.forEach({ documet in
                fetchPost(withPostId: documet.documentID) { post in
                    posts.append(post)
                    completion(posts)
                }
            })
    ...
    static func updateUserFeedAfterFollowing(user: User) {
        ...
        query.getDocuments { snapshot, error in
            ...
            docIDs.forEach { id in
                COLLECTION_USERS.document(user.uid).collection("user-feed").document(id).setData([:])
            }
        }
    }
}
